...
// Define file sources.
var sassFrontendSource        = [ 'dev/scss/wpbooklist-affiliates-main-frontend.scss' ];
var sassFrontendSourcePartial = [ 'dev/scss/_wpbooklist-affiliates-frontend-ui.scss' ];
var sassBackendSource         = [ 'dev/scss/wpbooklist-affiliates-main-admin.scss' ];
var sassBackendSourcePartial  = [ 'dev/scss/_wpbooklist-affiliates-backend-ui.scss' ];
...
gulp.task( 'concatAdminJs', function() {
	gulp.src(jsBackendSource ) // use jsSources
		.pipe(concat( 'wpbooklist_affiliates_admin.min.js' ) ) // Concat to a file named 'script.js'
...
gulp.task( 'concatFrontendJs', function() {
	gulp.src(jsFrontendSource ) // use jsSources
		.pipe(concat( 'wpbooklist_affiliates_frontend.min.js' ) ) // Concat to a file named 'script.js'
		.pipe(uglify() ) // Uglify concatenated file
		.pipe(gulp.dest( 'assets/js' ) ); // The destination for the concatenated and uglified file
});
...
gulp.task( 'copyconfig', function () {
	gulp.src([ './wpbooklistconfig.ini' ], {base: './'}).pipe(gulp.dest( '../wpbooklist-affiliates_dist' ) );
});
